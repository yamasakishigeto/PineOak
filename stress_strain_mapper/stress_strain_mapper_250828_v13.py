...
def build_mapper_with_control_figure(geo, strain, stress, init_mode="click"):
    ...
    # =========================
    # Figure3: Hardening Rate Map 表示機能
    # =========================
    from matplotlib.widgets import TextBox

    fig_hard, ax_hard = plt.subplots(figsize=(5, 5))
    fig_hard.canvas.manager.set_window_title("Hardening Rate Map")
    ax_hard.set_title("Hardening Rate Map")

    # n, m 入力欄（別窓）
    fig_input = plt.figure(figsize=(3.5, 1.5))
    fig_input.canvas.manager.set_window_title("n-m Range Selector")

    ax_tb_n = fig_input.add_axes([0.15, 0.55, 0.3, 0.3])
    ax_tb_m = fig_input.add_axes([0.55, 0.55, 0.3, 0.3])
    tb_n = TextBox(ax_tb_n, "n", initial="1")
    tb_m = TextBox(ax_tb_m, "m", initial="3")

    def update_hardening(val=None):
        try:
            n = int(tb_n.text)
            m = int(tb_m.text)
            if n > m:
                n, m = m, n
            if m - n < 1 or m >= strain_vals.shape[1]:
                return

            mode = curve_mode_radio.value_selected  # 既存のラジオボタンから取得
            hardening_vals = np.full(len(x), np.nan)

            if mode == "point":
                for i in range(len(x)):
                    xx = strain_vals[i, n:m+1]
                    yy = stress_vals[i, n:m+1]
                    if np.any(np.isnan(xx)) or np.any(np.isnan(yy)):
                        continue
                    hardening_vals[i] = np.polyfit(xx, yy, 1)[0]
            else:
                gid2slope = {}
                for gid in unique_grains:
                    xx = grain_mean_strain[gid][n:m+1]
                    yy = grain_mean_stress[gid][n:m+1]
                    if np.any(np.isnan(xx)) or np.any(np.isnan(yy)):
                        gid2slope[gid] = np.nan
                    else:
                        gid2slope[gid] = np.polyfit(xx, yy, 1)[0]
                for i, gid in enumerate(grain_id):
                    hardening_vals[i] = gid2slope.get(gid, np.nan)

            ax_hard.clear()
            sc = ax_hard.scatter(x, y, c=hardening_vals, cmap='viridis', s=28, alpha=0.85)
            cbar = fig_hard.colorbar(sc, ax=ax_hard)
            cbar.set_label("Hardening Rate [GPa]")
            ax_hard.set_title(f"Hardening Rate Map (n={n}, m={m}, mode={mode})")
            fig_hard.canvas.draw_idle()

        except Exception as e:
            print("Hardening update error:", e)

    tb_n.on_submit(update_hardening)
    tb_m.on_submit(update_hardening)
