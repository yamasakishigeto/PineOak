...
try:
    ...
    if GPU_AVAILABLE:
        # RTX50xx世代等でTF32がデフォルト有効だと精度が落ちるため明示的に無効化
...
# =============================================================================
# GPU(PyTorch)実装
#   heaviside_gpu_prototype/ での検証(Stage1〜Stage3, golden data突合)を経て
#   移植した process_one_subset のGPU版。マスク付きNCCを
#   sumF/sumF2/sumG/sumG2/sumFG（共分散展開公式）に分解し、GEMM(matmul)化する。
#   アルゴリズム・出力仕様は process_one_subset と完全に同一。
# =============================================================================
class _GpuMaskCache:
    ...
    def __init__(self, theta_center, offset_max, device,
                 subset_size=None, theta_range=None, theta_step=None,
                 offset_step=None):
        subset_size = subset_size or SUBSET_SIZE
        theta_range = theta_range if theta_range is not None else THETA_RANGE
        theta_step = theta_step or THETA_STEP
        offset_step = offset_step or OFFSET_STEP
        ...
        for i, (theta, offset) in enumerate(combos):
            mA, _ = make_heaviside_mask(subset_size, theta, offset)
            mask_A_list[i] = mA.flatten().astype(np.float32)

        mask_A_t = torch.from_numpy(mask_A_list).to(device)      # (n_combo, 961)
        mask_B_t = 1.0 - mask_A_t                                 # 補集合
        self.combined_mask = torch.cat([mask_A_t, mask_B_t], dim=0)  # (2*n_combo, 961)
        self.N = self.combined_mask.sum(dim=1)
        self.device = device
        self.subset_size = subset_size


_GPU_SEARCH_RANGE = SEARCH_RANGE
_GPU_K_FULL = (2 * _GPU_SEARCH_RANGE + 1) ** 2  # 49


def _gpu_is_full_range(ref_shape, deformed_shape, cx, cy, subset_size):
    half = subset_size // 2
    if (cy - half < 0 or cy + half >= ref_shape[0] or
            cx - half < 0 or cx + half >= ref_shape[1]):
        return False
    lo, hi = -_GPU_SEARCH_RANGE, _GPU_SEARCH_RANGE
...
def _gpu_estimate_bytes_per_subset(n_combo2, k=_GPU_K_FULL, elem_bytes=4):
    fm = n_combo2 * SUBSET_SIZE * SUBSET_SIZE
    g = k * SUBSET_SIZE * SUBSET_SIZE
    inter = 6 * k * n_combo2
    return elem_bytes * (fm + g + inter) * 1.3  # 1.3倍の安全マージン


def _gpu_choose_batch_size(n_combo2, n_subsets, device, safety_fraction=0.4):
    if device.type != "cuda":
        return max(1, n_subsets)
    free_bytes, _total = torch.cuda.mem_get_info(device)
    budget = free_bytes * safety_fraction
    per_subset = _gpu_estimate_bytes_per_subset(n_combo2)
    batch_size = max(1, int(budget // per_subset))
    return min(batch_size, n_subsets)


def _gpu_process_one_subset(ref, deformed, cx, cy, u_init, v_init,
                             mask_cache, min_side_px):
    """画像端でshift候補49件が確保できないサブセット用のフォールバック(1件ずつGEMM)。"""
    device = mask_cache.device
    ...
    for dv in range(dv_init_int - _GPU_SEARCH_RANGE, dv_init_int + _GPU_SEARCH_RANGE + 1):
        for du in range(du_init_int - _GPU_SEARCH_RANGE, du_init_int + _GPU_SEARCH_RANGE + 1):
    ...
    ref_patch = ref[cy - half:cy + half + 1, cx - half:cx + half + 1].astype(np.float32)
    F = torch.from_numpy(ref_patch.flatten()).to(device)
    G = torch.from_numpy(np.stack(patches).reshape(len(patches), -1)
                          .astype(np.float32)).to(device)

    M = mask_cache.combined_mask
    N = mask_cache.N
    ...
    denomF = (sumF2 - sumF * sumF / N).clamp(min=0.0)
    denomG = (sumG2 - sumG * sumG / N.unsqueeze(0)).clamp(min=0.0)
    ...
    best_ncc, best_shift_idx = ncc.max(dim=0)
    ...
    valid_combo = ((N_A >= min_side_px) & (N_B >= min_side_px) &
                   (ncc_A >= NCC_THRESHOLD) & (ncc_B >= NCC_THRESHOLD) &
    ...
    return _gpu_finalize_result(ref, deformed, cx, cy, subset_size,
                                 theta_best, offset_best, ref_patch,
                                 du_A, dv_A, du_B, dv_B,
                                 float(ncc_A[best_i].item()), float(ncc_B[best_i].item()))


def _gpu_finalize_result(ref, deformed, cx, cy, subset_size, theta_best, offset_best,
                          ref_patch, du_A, dv_A, du_B, dv_B, ncc_A_val, ncc_B_val):
    """best (theta,offset,shift) から subpixel_fit(CPU) を呼んで最終結果を組み立てる。
    process_one_subset の戻り値仕様と完全に同一のdictを返す。"""
    mask_A, mask_B = make_heaviside_mask(subset_size, theta_best, offset_best)

    ref_masked_A = ref_patch[mask_A]
    f_A = ref_masked_A - ref_masked_A.mean()
    f_norm_A = float(np.sqrt(np.sum(f_A ** 2)))
    u_A, v_A = subpixel_fit(ref, deformed, cx, cy, subset_size, mask_A,
                             f_A, f_norm_A, du_A, dv_A)

    ref_masked_B = ref_patch[mask_B]
    f_B = ref_masked_B - ref_masked_B.mean()
    f_norm_B = float(np.sqrt(np.sum(f_B ** 2)))
    u_B, v_B = subpixel_fit(ref, deformed, cx, cy, subset_size, mask_B,
    ...
    return dict(
        ...
        u_A=u_A, v_A=v_A, u_B=u_B, v_B=v_B,
        ncc_A=ncc_A_val, ncc_B=ncc_B_val,
...
def _gpu_process_batch(ref, deformed, subset_list, mask_cache, min_side_px):
    """subset_list: [(cx,cy,u_init,v_init), ...]  全件shift候補49通り前提でバッチ処理する。"""
    device = mask_cache.device
    subset_size = mask_cache.subset_size
    half = subset_size // 2
    S = len(subset_list)
    ...
    for cx, cy, u_init, v_init in subset_list:
        ...
        dv0_list.append(dv0)
        patches = []
        for dv in range(dv0 - _GPU_SEARCH_RANGE, dv0 + _GPU_SEARCH_RANGE + 1):
            for du in range(du0 - _GPU_SEARCH_RANGE, du0 + _GPU_SEARCH_RANGE + 1):
                cy_def, cx_def = cy + dv, cx + du
                patches.append(deformed[cy_def - half:cy_def + half + 1,
                                         cx_def - half:cx_def + half + 1])
        G_list.append(np.stack(patches).reshape(k, -1))
    ...
    sumF = F_batch @ M.T
    sumF2 = (F_batch * F_batch) @ M.T
    sumG = torch.einsum("skp,cp->skc", G_batch, M)
    sumG2 = torch.einsum("skp,cp->skc", G_batch * G_batch, M)
    FM = F_batch.unsqueeze(1) * M.unsqueeze(0)
    sumFG = torch.bmm(G_batch, FM.transpose(1, 2))

    numer = sumFG - sumF.unsqueeze(1) * sumG / N.view(1, 1, -1)
    denomF = (sumF2 - sumF * sumF / N.view(1, -1)).clamp(min=0.0)
    denomG = (sumG2 - sumG * sumG / N.view(1, 1, -1)).clamp(min=0.0)
    ...
    best_ncc, best_shift_idx = ncc.max(dim=1)
    n_combo = mask_cache.n_combo
    ncc_A, ncc_B = best_ncc[:, :n_combo], best_ncc[:, n_combo:]
    N_A, N_B = N[:n_combo], N[n_combo:]
    ...
    valid_combo = ((N_A.view(1, -1) >= min_side_px) & (N_B.view(1, -1) >= min_side_px) &
                   ...
                   torch.isfinite(ncc_A) & torch.isfinite(ncc_B))
    score = torch.where(valid_combo, ncc_A + ncc_B, torch.full_like(ncc_A, float("-inf")))
    best_i = torch.argmax(score, dim=1)
    ...
    for s in range(S):
        ...
        # 各サブセット固有のdu0,dv0を基準にインデックスを復元
        # (patches は dv 外側ループ・du 内側ループの順で積んでいるため row幅=2*SEARCH_RANGE+1で割る)
        row_w = 2 * _GPU_SEARCH_RANGE + 1
        dv_off_A, du_off_A = divmod(shift_idx_A, row_w)
        dv_off_B, du_off_B = divmod(shift_idx_B, row_w)
        du_A = du0_list[s] - _GPU_SEARCH_RANGE + du_off_A
        dv_A = dv0_list[s] - _GPU_SEARCH_RANGE + dv_off_A
        du_B = du0_list[s] - _GPU_SEARCH_RANGE + du_off_B
        dv_B = dv0_list[s] - _GPU_SEARCH_RANGE + dv_off_B

        results.append(_gpu_finalize_result(
            ref, deformed, cx, cy, subset_size, theta_best, offset_best,
            ref_patches[s], du_A, dv_A, du_B, dv_B,
            float(ncc_A[s, i].item()), float(ncc_B[s, i].item())))
    return results


def _gpu_process_grain(ref, deformed, subset_list, mask_cache, min_side_px,
                        safety_fraction=0.4):
    """subset_list: [(cx,cy,u_init,v_init), ...]。フル49件のものはバッチ化、
    画像端で欠けるものは1件ずつのGEMMにフォールバックする。
    戻り値: subset_listと同じ順のdictリスト。"""
    device = mask_cache.device
    full_idx, boundary_idx = [], []
    for idx, (cx, cy, u_init, v_init) in enumerate(subset_list):
        if _gpu_is_full_range(ref.shape, deformed.shape, cx, cy, mask_cache.subset_size):
            full_idx.append(idx)
        else:
    ...
    for b in boundary_idx:
        ...
        results[b] = _gpu_process_one_subset(ref, deformed, cx, cy, u_init, v_init,
                                              mask_cache, min_side_px)

    if full_idx:
        n_combo2 = mask_cache.combined_mask.shape[0]
        batch_size = _gpu_choose_batch_size(n_combo2, len(full_idx), device,
                                             safety_fraction)
        for start in range(0, len(full_idx), batch_size):
            ...
            chunk_results = _gpu_process_batch(ref, deformed, chunk_subsets,
                                                mask_cache, min_side_px)
...
def run_heaviside_dic(xlsx_path, ref_path, def_path, label,
                      ...
                      grain_thr_map=None,
                      use_gpu=False, gpu_safety_fraction=0.4):
    """
    ...
    use_gpu            : True で GPU(PyTorch)実装を使用する。既定はFalse（既存動作を維持）。
                          torch/CUDAが利用できない環境でTrueを指定するとRuntimeErrorになる。
    gpu_safety_fraction : GPUバッチ処理時に使用する空きVRAMの割合（既定0.4）
    """
    if use_gpu and not GPU_AVAILABLE:
        raise RuntimeError(
            "use_gpu=True が指定されましたが torch/CUDA が利用できません。"
            " GPU環境で実行するか use_gpu=False (既定) を指定してください。")
    if offset_max  is None: offset_max  = OFFSET_MAX
    if min_side_px is None: min_side_px = MIN_SIDE_PX
    ...
    if use_gpu:
        ...
        print(f"  GPU実行: {torch.cuda.get_device_name(device)}")
        # マスク行列(theta×offset×2面)はgrainごとに定数なので1回だけ構築して使い回す
        gpu_mask_caches = {}
        for gid in target_gids:
            gpu_mask_caches[gid] = _GpuMaskCache(grain_theta_map[gid], offset_max, device)
        print(f"  マスクキャッシュ生成完了: {len(gpu_mask_caches)} 粒分")

        by_grain_subsets = {gid: [] for gid in target_gids}
        by_grain_order = {gid: [] for gid in target_gids}
        for idx, (cx, cy, u0, v0, theta_c, gid) in enumerate(subset_list):
            by_grain_subsets[gid].append((cx, cy, u0, v0))
            by_grain_order[gid].append(idx)

        results = [None] * len(subset_list)
        for gid in target_gids:
            gsubsets = by_grain_subsets[gid]
            if not gsubsets:
                continue
            gresults = _gpu_process_grain(ref, deformed, gsubsets,
                                           gpu_mask_caches[gid], min_side_px,
                                           safety_fraction=gpu_safety_fraction)
            for idx, r in zip(by_grain_order[gid], gresults):
                results[idx] = r
    elif JOBLIB_AVAILABLE and n_jobs != 1:
